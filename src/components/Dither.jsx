...
function DitheredWaves({ waveSpeed, waveFrequency, waveAmplitude, waveColor, colorNum, pixelSize, disableAnimation, enableMouseInteraction, mouseRadius }) {
  ...
  const mouseRef = useRef(new THREE.Vector2(0, 0)); // Start at 0,0
  ...
  // --- PERBAIKAN UTAMA: WINDOW EVENT LISTENER ---
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (!enableMouseInteraction) return;
      const dpr = gl.getPixelRatio();
      // Update mouseRef langsung dari posisi window client
...
export default function Dither({ waveSpeed = 0.05, waveFrequency = 3, waveAmplitude = 0.3, waveColor = [0.5, 0.5, 0.5], colorNum = 4, pixelSize = 2, disableAnimation = false, enableMouseInteraction = true, mouseRadius = 1 }) {
  return (
    <Canvas
      ...
      // Tambahkan pointerEvents: none di sini agar klik tembus ke tombol di bawahnya
      // Tapi karena kita pakai window listener, mouse tetap terdeteksi!
      style={{
        width: '100%',
        height: '100%',
        position: 'absolute',
